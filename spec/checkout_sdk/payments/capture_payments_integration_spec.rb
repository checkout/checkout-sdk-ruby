  ...
  describe '.capture_payment' do
    context 'when capture a payment' do
      it 'make full capture' do
        ...
        capture_response = default_sdk.payments.capture_payment(payment_response.id, request)
      ...
      it 'make full capture idempotent' do
        ...
        capture_response_1 = default_sdk.payments.capture_payment(payment_response.id, request, idempotency_key)
        assert_response(capture_response_1, %w[reference
                                     action_id
                                     _links])

        capture_response_2 = default_sdk.payments.capture_payment(payment_response.id, request, idempotency_key)
